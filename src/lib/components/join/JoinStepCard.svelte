...
<article
	...
	class="scroll-mt-24 rounded-3xl border border-border bg-bg-surface p-5 shadow-sm
		transition-all duration-200 sm:p-6
		{step.enabled ? 'opacity-100' : 'opacity-60'}"
	aria-labelledby={`join-step-title-${step.id}`}
>
	<div class="grid gap-5 lg:grid-cols-[4.5rem_minmax(0,1fr)_9rem] lg:items-start">
		<div
			class="flex h-14 w-14 items-center justify-center rounded-2xl border
				border-gold-light bg-gold-tint text-lg font-bold text-gold-text select-none"
			aria-hidden="true"
		>
			{index + 1}
		</div>
		...
		<div class="min-w-0">
			<h3 id={`join-step-title-${step.id}`} class="text-xl font-bold">
				{step.title}
			</h3>
			<p class="mt-3 leading-8 text-text-secondary">
				{step.description}
			</p>
				...
				<div class="mt-5 flex flex-wrap items-center gap-3">
					{#if step.action}
						<a
							...
							class="inline-flex items-center gap-2 rounded-xl bg-indigo px-5 py-3
					...
					{#if index === 0}
						<span
							class="rounded-full border border-gold-light bg-gold-tint px-4 py-2
			...
			{#if step.note}
				<div class="mt-5 rounded-2xl border border-border bg-bg-page p-4 text-text-secondary">
					<p class="font-semibold text-text-primary">{step.note.title}</p>
		...
		<label
			class="inline-flex w-fit items-center gap-3 rounded-2xl border border-border
		...
		>
			<input
				type="checkbox"
				class="h-5 w-5 accent-indigo disabled:cursor-not-allowed"
				checked={step.completed}
				disabled={!step.enabled}
				onchange={(event) => onStepChange(index, event.currentTarget.checked)}
			/>
			<span class="select-none">{step.completed ? '已完成' : '勾選完成'}</span>
			{#if step.completed}
