<script lang="ts">
	...
	let { completedSteps, totalSteps, progressPercent }: Props = $props();
</script>
...
>
	<div class="join-page-header-grid grid gap-8 lg:grid-cols-[minmax(0,1fr)_minmax(280px,360px)]">
		<div class="join-page-header-copy">
			<p class="join-eyebrow mb-3 text-sm font-semibold tracking-[0.2em] text-gold-text uppercase">
				Start Learning
			</p>
		...
		<div
			...
			aria-label="加入流程完成進度"
		>
			<div class="join-progress-summary flex items-end justify-between gap-4" aria-live="polite">
				<p class="join-progress-count text-lg font-semibold text-indigo-darkest">
					{completedSteps} / {totalSteps} 完成
				</p>
				<p class="join-progress-percent text-3xl font-bold text-gold-text">{progressPercent}%</p>
			</div>
			<div
